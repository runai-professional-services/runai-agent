...
class FailureAnalyzerConfig(FunctionBaseConfig, name="runai_failure_analyzer"):
    ...
    lookback_days: int = Field(
        default=7, description="How many days of history to analyze"
    )

    pattern_threshold: int = Field(
        default=3, description="Minimum occurrences to identify a pattern"
    )

    # Remediation configuration
    enable_auto_remediation: bool = Field(
        default=False, description="Enable automatic remediation (with confirmation)"
    )

    allowed_projects: List[str] = Field(
        default=["*"], description="Whitelisted projects (use ['*'] for all)"
    )
...
class FailureDatabase:
    ...
    def _init_db(self):
        ...
        """)
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_run_history_ended ON job_run_history(ended_at)"
        )
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_run_history_project ON job_run_history(project)"
        )
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_run_history_status ON job_run_history(status)"
        )

        # Create indexes for performance
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_timestamp ON failure_events(timestamp)"
        )
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_project ON failure_events(project)"
        )
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_failure_type ON failure_events(failure_type)"
        )
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_node ON failure_events(node_name)"
        )
    ...
    def record_failure(self, failure_data: Dict[str, Any]) -> tuple[int, bool]:
        ...
        if existing:
            ...
            conn.close()
            logger.debug(
                f"⚠️  Duplicate failure detected for {job_name} - updated existing record #{failure_id}"
            )
    ...
    def get_best_solutions(
        self, failure_type: str, limit: int = 5
...
class FailurePatternAnalyzer:
    ...
    def _analyze_time_patterns(self, failures: List[Dict]) -> List[Dict]:
        ...
        if hour_failures:
            ...
            if max_hour_failures >= 5:
                peak_hours = [
                    h
                    for h, count in hour_failures.items()
                    if count >= max_hour_failures * 0.8
                ]
                patterns.append(
                    {
                        "type": "time_pattern",
    ...
    def _generate_recommendations(self, analysis: Dict) -> List[str]:
        ...
        for pattern in analysis.get("patterns", []):
            if pattern.get("type") == "project_pattern" and pattern.get("severity") in [
                "high",
                "critical",
            ]:
        ...
        for corr in analysis.get("correlations", []):
            if (
                corr.get("type") == "image_correlation"
                and corr.get("failure_count") >= 5
            ):
                recommendations.append(
        ...
        if not recommendations:
            recommendations.append(
                "✅ No critical patterns detected. System appears healthy."
            )
...
async def runai_failure_analyzer(config: FailureAnalyzerConfig, builder: Builder):
    ...
    async def _analyze_patterns(project: Optional[str], days: int) -> str:
        ...
        logger.info(
            f"🔍 Analyzing failure patterns (last {days} days, project={project or 'all'})"
        )
        ...
        if analysis["patterns"]:
            for pattern in analysis["patterns"]:
                if pattern["type"] == "project_pattern":
                    ...
                    report += (
                        f"   - Top failure types: {pattern['top_failure_types']}\n\n"
                    )
...
"""
    ...
    async def _get_remediation(
        failure_type: str, job_name: Optional[str], project: Optional[str]
    ) -> str:
        """Get remediation suggestions for a specific failure"""

        context = {"job_name": job_name, "project": project}
