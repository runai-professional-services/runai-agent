...
class RunaiKubectlTroubleshootConfig(
    FunctionBaseConfig, name="runai_kubectl_troubleshoot"
):
...
async def runai_kubectl_troubleshoot(
    config: RunaiKubectlTroubleshootConfig, builder: Builder
):
    ...
    async def _troubleshoot_fn(job_name: str, job_project: str) -> str:
        ...
        try:
            ...
            if (
                ...
                and job_project not in config.allowed_projects
            ):
                return f"""
❌ **Access Denied**
...
"""
            ...
            diagnosis = analyze_job_issues(
                pod_info,
                logs if config.include_logs else "",
                events if config.include_events else "",
            )
            response += diagnosis
...
"""
    ...
    async def get_job_status_info(job_name: str, job_project: str) -> str:
        ...
        try:
            ...
            # Search for workload
            workload_uuid, workload_type = _search_workload_by_name_helper(
                client, job_name
            )
            ...
            type_mapping = {
                "training": lambda: client.workloads.trainings.get_training(
                    workload_uuid
                ),
                "workspace": lambda: client.workloads.workspaces.get_workspace(
                    workload_uuid
                ),
                "distributed": lambda: client.workloads.distributed.get_distributed(
                    workload_uuid
                ),
            }
            ...
            types_to_try = (
                [workload_type.lower()]
                if workload_type
                else ["training", "workspace", "distributed"]
            )
...
"""
            ...
            if any(
                keyword in events_output.lower()
                for keyword in ["error", "failed", "warning", "backoff", "crash"]
            ):
