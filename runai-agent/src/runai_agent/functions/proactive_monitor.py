...
class RunaiProactiveMonitorConfig(FunctionBaseConfig, name="runai_proactive_monitor"):
    ...
    poll_interval_seconds: int = Field(
        default=60, description="How often to check job status (default: 60 seconds)"
    )
    ...
    max_alerts_per_job: int = Field(
        default=1, description="Maximum alerts to send per job (prevents spam)"
    )


@register_function(config_type=RunaiProactiveMonitorConfig)
async def runai_proactive_monitor(
    config: RunaiProactiveMonitorConfig, builder: Builder
):
    """
...
"""
        ...
        if action == "start":
            ...
            return await _start_monitoring(
                project, run_minutes, defaulted=(duration_minutes == 0)
            )

        return f"❌ Invalid action '{action}'. Use 'start' or 'status'."

    async def _start_monitoring(
        project_filter: Optional[str], duration_minutes: int, defaulted: bool = False
    ) -> str:
...
"""
        ...
        logger.info(
            ...
            + (" (default for chat/CLI)" if defaulted else "")
        )
        ...
        try:
            ...
            while True:
                check_count += 1
                logger.info(
                    f"🔄 Monitoring check #{check_count} at {datetime.now().strftime('%H:%M:%S')}"
                )
                ...
                if duration_minutes > 0:
                    ...
                    if elapsed >= duration_minutes:
                        logger.info(
                            f"✅ Monitoring duration reached ({duration_minutes} minutes)"
                        )
                        break
...
"""
    ...
    async def _fetch_workloads(
        client, project_filter: Optional[str]
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            all_workloads = (
                workloads_data.get("workloads", [])
                if isinstance(workloads_data, dict)
                else []
            )
            ...
            if project_filter and project_filter != "*":
                ...
                if project_filter:
                    filtered = [
                        w for w in filtered if w.get("projectName") == project_filter
                    ]
    ...
    async def _check_workload(workload: Dict[str, Any], client, builder: Builder):
        ...
        if config.monitor_only_failed and phase not in [
            "Failed",
            "Error",
            "ImagePullBackOff",
            "CrashLoopBackOff",
        ...
        if is_failure:
            logger.warning(
                f"🔴 FAILURE DETECTED: {job_name} in {job_project} - Phase: {phase}"
            )
            ...
            if job_uuid in _alerted_jobs:
                if _alerted_jobs[job_uuid] >= config.max_alerts_per_job:
                    logger.debug(
                        f"   Skipping alert (already sent {_alerted_jobs[job_uuid]} alerts)"
                    )
                    return
    ...
    async def _record_job_run_to_history(
        workload: Dict[str, Any], pod_info: Optional[Dict[str, Any]]
    ):
        ...
        db_path = os.environ.get(
            "RUNAI_FAILURE_DB_PATH", "/tmp/runai_failure_history.db"
        )
    ...
    async def _auto_troubleshoot(
        job_name: str, job_project: str, builder: Builder
    ...
    async def _record_failure_to_db(
        ...
        try:
            ...
            failure_data = {
                ...
                "logs_snippet": pod_info.get("logs_snippet", "")[:2000]
                if pod_info.get("logs_snippet")
                else None,
                "events_snippet": pod_info.get("events_snippet", "")[:2000]
                if pod_info.get("events_snippet")
                else None,
            ...
            if is_new:
                logger.info(
                    f"✓ Recorded NEW failure #{failure_id} to database for pattern analysis"
                )
                ...
                logger.info(
                    f"🔄 Updated existing failure #{failure_id} (still failing, within 1 hour window)"
                )
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        logger.warning(
                            f"⚠️ Slack notification failed: HTTP {response.status}"
                        )
