...
def _search_workload_by_name_helper(
    client, job_name_to_search: str, project_id: str = None, cluster_id: str = None
):
    """
    ...
    try:
        cmd = [
            ...
            "--all-namespaces",
            "-l",
            f"workloadName={job_name_to_search}",
            "-o",
            "jsonpath={.items[0].metadata.labels['run\\.ai/top-owner-uid']}",
        ]
        result = subprocess.run(
            cmd, capture_output=True, text=True, timeout=5, env=kubectl_env
        )
    ...
    try:
        ...
        for workload in workloads:
            ...
            logger.debug(
                f"Checking workload: {workload_name} (type: {workload_type}, id: {workload_id})"
            )

            if workload_name == job_name_to_search:
                logger.info(
                    f"✓ Found exact match: '{job_name_to_search}' (ID: {workload_id}, Type: {workload_type})"
                )
                return workload_id, workload_type

        logger.warning(
            f"No exact match found for '{job_name_to_search}' among {len(workloads)} workloads"
        )
        return None, None
    ...
    async def _fetch_examples(self) -> Dict[str, List[Dict[str, str]]]:
        ...
        try:
            ...
            if github_token:
                ...
                logger.info(
                    f"✓ Using GitHub token for authentication (length: {len(github_token)}, prefix: {github_token[:4]}...)"
                )
            else:
                ...
                for github_api_url in github_api_urls:
                    ...
                    try:
                        async with session.get(
                            github_api_url, timeout=aiohttp.ClientTimeout(total=10)
                        ) as response:
                            if response.status != 200:
                                error_body = await response.text()
                                logger.warning(
                                    f"Failed to fetch {dir_name} from GitHub: {response.status}"
                                )
                                    ...
                                    try:
                                        ...
                                        async with session.get(
                                            ...
                                            if file_response.status == 200:
                                                ...
                                                category = self._categorize_example(
                                                    filename
                                                )
                                                ...
                                                logger.debug(
                                                    f"Fetched example: {file.get('name')} from {dir_name}"
                                                )
                                    except Exception as e:
                                        logger.warning(
                                            f"Failed to fetch example {file.get('name')}: {e}"
                                        )
                                        continue
                    except Exception as e:
            ...
            logger.info(
                f"✅ Fetched {total_examples} runapy examples from GitHub (root + generated)"
            )
